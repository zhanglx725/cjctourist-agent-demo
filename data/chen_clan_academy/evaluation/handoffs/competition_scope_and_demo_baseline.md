...
这是一个以审核知识、确定性路线与可追踪游览状态为可信底座，并用“计划—生成—校验—回退”机制安全叠加角色化表达的陈家祠智能导游；比赛版已跑通古风书生主线和儿童、中性点位讲解的有限 Active 闭环。

## 3. 当前参赛状态

```text
branch: experiment/agent-orchestration-v2
baseline_commit: bf60a51
competition_tag: not_created
remote_sync_status: synchronized_with_origin
workspace_status: baseline_committed; protected_local_run_langgraph_studio.cmd_modified_only
...
active_takeover: limited_allowlist_only
```

人工 Studio 验证已确认：古风书生的路线规划、路线开场和审核点位讲解能够通过校验并 Active 展示；儿童友好、专业讲解和静听模式的角色问答 Shadow 候选可通过校验；失败时旧版安全正文仍可保留。未统一保存 Trace URL 的样本按 `trace_metadata: unavailable` 归档。
...
| `child` 儿童友好 | 旧链 | 旧链 | Active |
| `neutral` 中性清晰 | 旧链 | 旧链 | Active |
...
全 18 角色 Active、问答 Active、navigation Active、tour_closing Active、自由规划器接管、重规划 Active、语音交互、多场馆泛化和生产级全量灰度均未开放。
...
| 18 种角色目录 | 已完成并自动验证 | 角色策略层 | 可宣称“支持候选生成与 Shadow 验证”，不可宣称全部 Active |
| 古风书生 | 比赛有限 Active | 规划、开场、点位 | 主展示角色；事实、路线和状态仍由确定性系统提供 |
| 儿童友好 | 比赛有限 Active | 点位讲解 | 短句、直观解释，不降低安全要求 |
| 中性清晰 | 比赛有限 Active | 点位讲解 | 稳定兜底展示风格 |
| 专业讲解 | Shadow | 点位与问答候选 | 可展示审计，不宣称游客端 Active |
| 静听模式 | Shadow | 点位与问答候选 | 不新增问题、任务、拍照或互动要求 |
...
- 不现场轮测 18 种角色，不开放其他 15 种角色 Active；
- 不展示专业讲解、静听模式或粤派角色的 Active 接管；
...
- “系统已实现 18 种角色表达策略，并完成自动化验证与高风险角色 Shadow 抽样。”
- “比赛版已开放古风书生路线规划、开场和点位讲解的有限 Active。”
- “儿童友好与中性清晰已在白名单审核点位开放有限 Active。”
- “角色化问答已完成计划、生成、校验和审计链路，目前以 Shadow 方式运行。”
- “所有事实、路线和游览状态由确定性系统控制，角色模型只改变表达。”
- “完整回归 1118/1118 通过，角色失败会自动回退旧版安全正文。”

### 禁止使用

- “18 种角色已全部上线/全部 Active。”
...
- 演示视频优先使用古风书生主线；儿童友好作为差异化补充；中性清晰作为稳定兜底。
