...
## D5-B 学习说明：为什么“有打卡数据”不等于可以向游客推荐

**解决的问题**：打卡卡、姿势模板和平台观察都可能包含草稿、编辑建议或尚未复核的现场条件。若直接展示，会把未核验的安全、位置或“热门”信息当成导游事实。

**真实流程**：原始打卡卡/姿势/平台观察 → D1 统一只读注册 → `photo_spot_validation.validate_photo_spot_cards()` 严格核验 → 未来 D6 才可能读取可用结果。当前没有一张卡越过门控，所以返回 `no_reviewed_photo_spot`，而不是编造替代建议。

**关键规则**：四项核验状态、审核人、审核日期、无阻塞问题、合法节点、可用姿势、有效来源和对象关联缺一不可；平台观察不能成为运行证据，`editorial_recommended` 也不能变成“热门”。这是“失败关闭”而非“尽量可用”的安全策略。

**已实现并待本机验证**：只读校验器与定向测试。**仅保留接口**：`photo_spot_availability()` 的 D6 返回结构。**尚未实现**：游客问答、路线推荐、姿势展示、拍照引导和任何真实前端。

**答辩一分钟说法**：我们没有把打卡数据直接塞进导游回复，而是把它当作高风险体验内容单独门控。系统先验证位置、安全、内容、来源、人工审核和引用完整性；任一项不足就关闭。这样做的代价是当前可能没有可推荐打卡点，但它避免了把草稿建议、平台线索或未核验场地条件误说给游客。D5-B 因此是安全上线前的资格层，D6 才是通过门控后面向游客的调用层。
