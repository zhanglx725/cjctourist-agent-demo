...
明确说出审核点位并要求重规划时，执行顺序同样是安全条件：先通过 `resolve_reviewed_node()` 得到唯一 `node_id`，再经 A1 写入真实 `current_stop_id`，清除旧位置对应的候选，最后才建立新的剩余时间确认。反过来先根据旧当前位置做候选，会制造 `origin mismatch`。相反，若游客说自己在“没标名字的小院”或“没有标识的地方”，系统不能把旧当前位置、旧 pending 或默认入口冒充新起点；必须以 `unresolved_replan_origin` 澄清，并保持路线和画像不变。
...
没有地点的表达也不是无限制可执行：只有活跃路线处于导航阶段、且存在唯一 `pending_stop_id` 时，才允许把“我已经到目的地了”等泛化到达绑定到该 pending。高频完成体“我已经抵达这里了”“我人到了”“终于走到了”等先由确定性解析处理，不能依赖模型恰好产出候选。前往意愿、途中状态、否定、假设、疑问、第三人称和混合操作先被原话安全门拒绝。到达非 pending 点位仍是 P1-11 的偏航：记录真实当前位置、询问剩余时间，不写 visited，也不直接替换正式路线。

另有一个只读的 `looks_like_arrival_control()` 仲裁护栏：它不解析节点、更不执行状态写入；其唯一职责是把“像游客位置变化控制、但尚不能安全确认”的输入留在澄清通道，防止语义模型无候选、非法 JSON 或低置信度时回退为自由 RAG。时间条件知识问句（如“到达月台后能看到什么”）先被排除，继续走正常知识问答。
...
## P0-03 / CA-00 学习说明：为什么先冻结行为，再引入 Planner 与 Tools
