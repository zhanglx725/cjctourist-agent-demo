...
> 2026-08-02 执行同步：当前 `main == origin/main == fe84be2` 且工作树干净。P0 定向安全/游客输出矩阵已记录 59 项通过，但完整回归最新记录为 757 项、16 个失败、1 个错误；因此当前仍在“基线与正确性收口”，不得开始受控 Agent、语音、多语言、附近推荐或学术工作区生产接入。P1-12C1 高风险自然到达表达已提交待本机/Studio 复测；P1-04 空间别名继续失败关闭，等待数据负责人决策。
...
> D7 状态更新（待本机与 LangSmith 验证）：D1--D6 的综合验收矩阵已建立于 `data/chen_clan_academy/evaluation/d_stage_acceptance_cases_v1.yaml`，固定编号为 `dst_acc_001`--`dst_acc_017`。在完整回归、固定多轮 LangSmith 场景及人工审阅完成前，D 阶段保持“已实现待验证”，不得表述为已冻结或已完成。
...
## 阶段 B：点位讲解编排器（B1、B2、B3 已实现并验证，B4 待验收）

B1 新增 `guide_program_planner.py`，只决定“到站后从本点审核文物中讲哪 1–3 件”，输出稳定 `StopProgram`；不改变路线。项目负责人已完成 B1 相关 112 项及完整 161 项本机回归，均为 `OK`。B2 只分配路线已经交给本站的讲解内容预算，按兴趣与相关性接近时的工艺/题材多样性选择对象，并保留未分配内容时间；不占用步行时间。项目负责人已完成完整 166 项本机回归，耗时 1.740 秒，结果为 `OK`。B3 的 `stop_guidance` 按 StopProgram 的 `rag_query_hints` 复用既有 RAG，输出带来源的本点讲解，且不自动调用 `explanation_finished` 或写入 TourState；项目负责人已完成完整 173 项本机回归，耗时 1.775 秒，结果为 `OK`。B4 已新增离线闭环验收，待项目负责人完成本机回归与 LangSmith 真实链路检查。论文、比较、术语和打卡卡不阻塞 B，后续仅通过空的 `card_id` 插槽定向增强。
...
### P1-20 对象故事证据边界（`verified_fixed`）

规划前后的“明确审核对象 + 故事/人物/情节”统一经过对象级证据门控：工艺总述不能单独扩写为对象传说；精确 `08` 对象证据才可输出故事。游客若显式限制为工艺资料，系统只做范围澄清，不隐式改用对象资料或 LLM 补写。该问答链路只读，不改变 TourState 或路线。

同名对象消歧子项：可唯一的候选可继续精确对象取证；审核实体在游客可公开字段上仍不可区分且没有 canonical/alias 审核关系时，系统展示候选类别但失败关闭。项目负责人已确认木雕《踏雪寻梅》`orn_051/orn_052` 为同一物理对象，构建期只投影 canonical `orn_051`；木雕与石雕候选现均可精确取证。

2026-07-31 已完成 LangSmith 新线程人工验收；本地归档未保留具体 thread ID/Trace URL，不以占位链接替代真实证据。

### P1-11 显式当前位置的后续重规划（已实现，待 LangSmith 验证）

中途到达仍只更新 `current_stop_id`，不写 visited；但活跃路线中明确自主到达非 pending 审核点位会被视为偏航，先确认游客本轮剩余时间，不能把初始总时长当作现场剩余时间。收到明确时间后才从该唯一物理位置生成候选并等待路线确认。候选展示明确列出路线起点与正式讲解停靠点，并验证路径首节点就是当前物理位置；两个确认阶段都禁止 `next_stop` 绕过。显式“从该点调整后续行程”仍是受控组合，不扩展为任意多意图部分执行。确认后才原子替换未完成路线；候选起点是 `current_stop_id` 的审计快照，游客移动后旧候选失效；取消不覆盖正式路线，导航继续从真实当前位置计算。
