...
def _craft_evidence_matches(entry: Mapping[str, Any], craft: str) -> bool:
    if _document_name(entry) != CRAFT_DOCUMENT or not _has_sources(entry):
        return False
...
def _ornament_evidence_matches(entry: Mapping[str, Any], item: SelectedItem) -> bool:
    if _document_name(entry) != ORNAMENT_DOCUMENT or not _has_sources(entry):
        return False
    # The item title is the audited identity boundary.  Content-only matching
    # can mention a similarly named object and must not become its evidence.
    return item.name in _title_parts(entry)
...
def build_guidance_evidence_bundle(
    ...
    for craft in crafts:
        if craft_status[craft] == "repeat":
            continue
        query = f"{craft} 定义 材料 技法 建筑位置 特点"
    ...
    source_ids = tuple(sorted({source for packet in (*craft_packets.values(), *ornament_packets.values()) for source in packet.source_ids}))
